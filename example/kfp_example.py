...
@dsl.component(
    ...
    packages_to_install=["numpy", "scikit-learn", "joblib", "pandas", "minio==7.1.14"],
)
def model_training(
...
@dsl.component(
    base_image="python:3.9",
    packages_to_install=["pandas", "joblib", "minio==7.1.14", "scikit-learn"],
)
def predict(
    X_test: Input[Dataset], trained_model: Input[Artifact], prediction: Output[Dataset]
):
    import joblib
    import pandas as pd
    from minio import Minio
...
@dsl.component(
    base_image="python:3.9", packages_to_install=["pandas", "scikit-learn", "numpy"]
)
def evaluate(
...
def pipeline():
    ...
    preprocess_data = preprocess_the_dataset(
        dataset_content=read_data.outputs["output_dataset"]
    )
    split = train_test_split(input_df=preprocess_data.outputs["out_data"])
    trained_model = model_training(
        X_train_input=split.outputs["X_train_artifact"],
        X_test_input=split.outputs["X_test_artifact"],
        y_train_input=split.outputs["y_train_artifact"],
    )  # Fixed typo here
    predicted_value = predict(
        X_test=trained_model.outputs["X_test_scaled"],
        trained_model=trained_model.outputs["model_output"],
    )
    evaluate(
        y_test=split.outputs["y_test_artifact"],
        predictions=predicted_value.outputs["prediction"],
    )  # Fixed typo here
...
def upload():
    ...
    client.fput_object(
        bucket_name,
        "data/housing.csv",
        Path(__file__).parent / "kfp_example_housing.csv",
    )
