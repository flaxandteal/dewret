from dewret.renderers.snakemake import RunDefinition
...
def mock_get_method_args(func):
    ...
    p1 = inspect.Parameter(
        name="param1", kind=inspect.Parameter.KEYWORD_ONLY, annotation=str
    )
    p2 = inspect.Parameter(
        name="param2", kind=inspect.Parameter.KEYWORD_ONLY, annotation=int
    )
    return inspect.Signature(parameters=[p1, p2])
...
@patch(
    "dewret.renderers.snakemake.get_method_rel_path",
    side_effect=mock_get_method_rel_path,
)
